...
async def set_schedule(message: Message):
    ...
    async def save_time(message: Message):
        ...
        try:
            ...
            datetime.strptime(time, "%H:%M")  # this will raise ValueError if invalid
            cursor.execute("INSERT INTO schedules (user_id, time) VALUES (?, ?)", (user_id, time))
            connect.commit()
            await message.answer(f"Время {time} успешно сохранено!")
        except ValueError:
...
async def send_reminder():
    ...
    for user_id, time in schedules:
        current_time = datetime.now().strftime("%H:%M")
        if current_time == time:  # Compare the current time to the scheduled time
            await bot.send_message(user_id, "Пора выполнить задачу!")
...
async def scheduler():
    ...
    while True:
        await aioschedule.run_pending()
        await asyncio.sleep(60)  # Sleep for 1 minute to avoid blocking the event loop


async def on_start():
    asyncio.create_task(scheduler())  # Run the scheduler in the background


@dp.message(Command("view_schedule"))
async def view_schedule(message: Message):
    user_id = message.from_user.id
    schedules = cursor.execute("SELECT task, time FROM schedules WHERE user_id = ?", (user_id,)).fetchall()
    if schedules:
        response = "Ваше расписание:\n" + "\n".join([f"{task} - {time}" for task, time in schedules])
...
async def delete_schedule(message: Message):
    try:
        ...
        await message.answer(f"Произошла ошибка при удалении задачи: {str(e)}")
...
async def update_schedule(message: Message):
    try:
        ...
        logging.error(f"Error in update_schedule: {e}")
        await message.answer(f"Произошла ошибка при обновлении времени: {str(e)}")
